...
import { authAPI, patentAPI, statsAPI } from './api';
...
export default function App() {
  const [screen, setScreen] = useState("login"); // login | register | dashboard | form | loading | results | formdetails | generating | formsready | account | history
  ...
  const [registerForm, setRegisterForm] = useState({ 
    ...
    department: "" // institution is set by backend as default
  });
...
Believe in the Possibilities`;
        ...
        <div style={{maxWidth:800,margin:"0 auto",padding:"40px 20px"}}>
          ...
          <div className="rc">
            ...
            <div style={{marginTop:16,padding:"12px",background:"rgba(76,175,80,.05)",borderRadius:8,fontSize:12,color:C.muted,textAlign:"center"}}>
              <strong>🤖 Powered by Gemini AI</strong> - All analysis uses Google's Gemini 3 Flash model
            </div>
          </div>
            ...
            <div style={{background:"rgba(43,95,143,.05)",padding:16,borderRadius:10,marginBottom:16}}>
              ...
              <div>
                ...
                <input 
                  ...
                  type="password" 
                  placeholder="AIza..."
                  value={registerForm.geminiKey}
                  onChange={e=>setRegisterForm(p=>({...p,geminiKey:e.target.value}))}
                />
              </div>
      ...
      <nav className="nav">
        ...
        <div style={{display:"flex",gap:9,alignItems:"center"}}>
          ...
          {isLoggedIn && screen !== "dashboard" && (
            <button className="btn" style={{fontSize:13,padding:"7px 14px"}} onClick={() => setScreen("dashboard")}>🏠 Home</button>
          )}
          {isLoggedIn && (
  ...
  if(screen==="loading") return (
    <div className="bg" style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:22}}>
      ...
      <div style={{color:C.muted,fontSize:14}}>Gemini AI is analyzing your invention...</div>
        ...
        <div style={{background:"#ffffff",border:`1px solid ${C.border}`,borderRadius:16,boxShadow:"0 2px 12px rgba(0,0,0,.06)",padding:"32px"}}>
          ...
          {step===3&&<>
            ...
            <div style={{display:"flex",gap:10,marginTop:14}}>
              ...
              <button className="btn" style={{flex:1}} onClick={analyze}>⚡ Analyze with Gemini AI</button>
            </div>
          </>}
        </div>
      </div>
